        ...
        **IMPLEMENT THIS FUNCTION**
        ...
        for i in range(self.num_agents):
            if random.random() < self.epsilon: #exploration (random action)
                action = self.action_spaces[i].sample()
            else: #explotation (best action)
                
                q_values = []

                for a in range(self.n_acts[i]):
                    value = [self.q_tables[i][str((obss[i], a))]]
    ...
    def learn(
        ...
        **IMPLEMENT THIS FUNCTION**
        ...
        for i in range(self.num_agents):
            ...
            state_key = str((obs, action))
            current_q = self.q_tables[i][state_key]
            ...
            if done:
                ...
                for a in range(self.n_acts[i]):
                    value = max([self.q_tables[i][str((next_obs, a))]])
                    next_q.append(value)
            ...
            #update table
            self.q_tables[i][state_key] = new_q
